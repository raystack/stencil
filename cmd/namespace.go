...
func NamespaceCmd(cdk *CDK) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ stencil namespace create -n goto
			$ stencil namespace view goto
		`),
...
func createNamespaceCmd(cdk *CDK) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			...
			$ stencil namespace create -n=goto -f=FORMAT_PROTOBUF -c=COMPATIBILITY_BACKWARD -d="Event schemas"
		`),
		RunE: func(cmd *cobra.Command, args []string) error {
			prompter := prompt.New()
			if id == "" {
...
func editNamespaceCmd(cdk *CDK) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: heredoc.Doc(`
			$ stencil namespace edit goto -f FORMAT_JSON -c COMPATIBILITY_BACKWARD -d "Hello message"
		`),
		RunE: func(cmd *cobra.Command, args []string) error {
			spinner := printer.Spin("")
			defer spinner.Stop()
